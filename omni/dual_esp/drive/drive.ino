...
int desiredMotor3Direction = 1; // 1 = forward, 0 = reverse
int desiredMotor4Direction = 1; // 1 = forward, 0 = reverse
...
// Set motors for omni-directional movement
void setMotorOmni(float xVal, float yVal, float rVal) {
  ...
  float maxValue = max(max(fabs(frontLeft), fabs(frontRight)),
  ...
  desiredLeftDirection = (frontLeft >= 0) ? 1 : 0;
  desiredRightDirection = (frontRight >= 0) ? 1 : 0;
  desiredMotor3Direction = (backLeft >= 0) ? 1 : 0;
  desiredMotor4Direction = (backRight >= 0) ? 1 : 0;
  Serial.printf("[DriveBoard] Omni => x=%.1f y=%.1f r=%.1f\n", xVal, yVal, rVal);
}

// Set motors for simple forward/backward/turn movement
void setMotorSimple(float speedPercent, const String& forwardBackward, float turnRatePercent) {
  if (emergencyStopActive) return;
...
void updateMotorSignals() {
  ...
  float finalMotor3PWM = desiredMotor3PWM * leftTune;
  float finalMotor4PWM = desiredMotor4PWM * rightTune;
